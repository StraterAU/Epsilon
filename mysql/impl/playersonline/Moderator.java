...
public class Moderator {
	...
	public static void login(Player p) {
		...
		if (!p.getRights().contains(PlayerRights.MODERATOR)) {
			return;
		}
		...
		GameServer.getLoader().getEngine().submit(() -> {
			try {
				PreparedStatement preparedStatement = login.getConnection().prepareStatement("INSERT INTO moderator (username) VALUES (?)");
	...
	public static void logout(Player p) {
		...
		GameServer.getLoader().getEngine().submit(() -> {
			try {
				PreparedStatement preparedStatement = login.getConnection().prepareStatement("DELETE FROM moderator WHERE username = ?");
