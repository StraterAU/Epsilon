...
public class Administrator {
	...
	public static void login(Player p) {
		...
		if (!p.getRights().contains(PlayerRights.ADMINISTRATOR)) {
			return;
		}
		...
		GameServer.getLoader().getEngine().submit(() -> {
			try {
				PreparedStatement preparedStatement = login.getConnection().prepareStatement("INSERT INTO administrator (username) VALUES (?)");
	...
	public static void logout(Player p) {
		...
		GameServer.getLoader().getEngine().submit(() -> {
			try {
				PreparedStatement preparedStatement = login.getConnection().prepareStatement("DELETE FROM administrator WHERE username = ?");
