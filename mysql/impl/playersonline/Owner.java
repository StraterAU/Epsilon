...
public class Owner {
	...
	public static void login(Player p) {
		...
		if (!p.getRights().contains(PlayerRights.OWNER)) {
			return;
		}
		...
		GameServer.getLoader().getEngine().submit(() -> {
			try {
				PreparedStatement preparedStatement = login.getConnection().prepareStatement("INSERT INTO owner (username) VALUES (?)");
	...
	public static void logout(Player p) {
		...
		GameServer.getLoader().getEngine().submit(() -> {
			try {
				PreparedStatement preparedStatement = login.getConnection().prepareStatement("DELETE FROM owner WHERE username = ?");
