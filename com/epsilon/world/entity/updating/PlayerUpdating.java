package com.epsilon.world.entity.updating;
...
import com.epsilon.model.UpdateFlag;
...
import com.epsilon.net.packet.ByteOrder;
...
import com.epsilon.net.packet.PacketBuilder.AccessType;
...
public class PlayerUpdating {
	...
	private static void updateChat(PacketBuilder builder, Player target) {
		...
		builder.put(target.getHighestRights().ordinal());
	...
	private static void updateAppearance(Player player, PacketBuilder out, Player target) {	
		...
		properties.put(appearance.getHciIcon());
		if (player.getNpcTransformationId() <= 0) {
			...
			if (equip[Equipment.CAPE_SLOT] > -1) {
				...
				if(equip[Equipment.CAPE_SLOT] == 14022 || equip[Equipment.CAPE_SLOT] == 14024 || equip[Equipment.CAPE_SLOT] == 14019) {
		...
		properties.putShort(target.getHighestRights().ordinal());
