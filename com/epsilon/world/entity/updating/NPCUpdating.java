...
public class NPCUpdating {
	...
	public static void update(Player player) {
		...
		for (Iterator<NPC> npcIterator = player.getLocalNpcs().iterator(); npcIterator.hasNext();) {
			...
			if (World.getNpcs().get(npc.getIndex()) != null && npc.isVisible(player) && player.getPosition().isWithinDistance(npc.getPosition()) && !npc.isNeedsPlacement()) {
		...
		for(NPC npc : World.getNpcs()) {
			if (player.getLocalNpcs().size() >= 79) //Originally 255
				break;
			if (npc == null || player.getLocalNpcs().contains(npc) || !npc.isVisible(player) || npc.isNeedsPlacement())
				continue;
