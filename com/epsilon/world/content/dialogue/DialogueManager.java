...
public class DialogueManager {
	...
	private static void showDialogue(Player player, Dialogue dialogue) {
		...
		case ITEM_STATEMENT:
			...
			break;
		case STATEMENT:
			sendStatement(player, dialogue.dialogue());
			break;
		case OPTION:
	...
	public static void sendStatement(Player p, String... lines) {
		...
		for(int i = 0; i <= lines.length; i++) {
			if(i == lines.length) {
				p.getPacketSender().sendString(id, "Click here to continue");
				break;
			}
			p.getPacketSender().sendString(id++, lines[i]);
		}
		p.getPacketSender().sendChatboxInterface(statementIds[lines.length - 1] - 1);
	}
