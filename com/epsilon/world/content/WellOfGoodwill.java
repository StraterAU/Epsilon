...
public class WellOfGoodwill {
	...
	public static void donate(Player player, int amount) {
		...
		if (getMissingAmount() <= 0) {
			...
			World.getPlayers().forEach(p -> p.getPacketSender().sendString("@or2@Well of Goodwill: @gre@Active", PlayerPanel.getLineId(7)));
		}
	}
	...
	public static void updateState() {
		if (STATE == WellState.FULL) {
			if(getMinutesRemaining() <= 0) {
				...
				World.getPlayers().forEach(p -> p.getPacketSender().sendString("@or2@Well of Goodwill: @red@Inactive", PlayerPanel.getLineId(7)));
				setDefaults();
			}
		}
	}
