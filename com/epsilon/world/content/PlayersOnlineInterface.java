...
public class PlayersOnlineInterface {
	...
	private static void sendInterfaceData(Player player) {
		...
		for(int i = 0; i < fakeCount; i++) {
			...
			if(p == null)
				continue;
			int rankId = p.getHighestRights().ordinal();
			if(rankId == 0) {
	...
	private static void updateInterface(Player player, int index) {
		...
		player.getPacketSender().sendString(57008, "Name: @whi@"+player2.getUsername()).sendString(57009, "Rank: @whi@"+Misc.formatText(player2.getHighestRights().name().toLowerCase())).
