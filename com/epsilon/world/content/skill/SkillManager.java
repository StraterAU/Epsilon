...
import com.epsilon.world.content.BrawlingGloves;
...
public class SkillManager {
	...
	public static int[] GOLDEN_MINING_SUIT = {  };
	...
	public SkillManager addExperience(Skill skill, int experience) {
		...
		if(skill == Skill.MINING) {
			...
			for(Item item : player.getEquipment().getItems()) {
				if(item != null && item.getId() >= 20787 && item.getId() <= 20791) {
					++multiplier;
				}
			}
			
			experience += (experience / 100) * multiplier;
			} else {
				byte multiplier = 0;
				...
				experience += (experience / 500) * multiplier;
			}
		...
		if (player.getGameMode() == GameMode.EXTREME) {
			...
			experience *= player.getHighestRights().getExperienceGainModifier();
		}
		if(player.isXpMessage() == false){
			if(player.hasRights(PlayerRights.UBER_DONATOR))
					player.getPacketSender().sendMessage("Your bonus has been multiplied by 100% since you're an uber donator.");
			else if(player.hasRights(PlayerRights.LEGENDARY_DONATOR))
					player.getPacketSender().sendMessage("Your bonus has been multiplied by 70% since you're a legendary donator.");
			else if(player.hasRights(PlayerRights.EXTREME_DONATOR))
				player.getPacketSender().sendMessage("Your bonus has been multiplied by 50% since you're an extreme donator.");
			else if(player.hasRights(PlayerRights.SUPER_DONATOR))
				player.getPacketSender().sendMessage("Your bonus has been multiplied by 25% since you're a super donator.");
		player.setXpMessage(true);
		}
		...
		int old = this.skills.experience[skill.ordinal()];
		...
		if(old < MAX_EXPERIENCE && this.skills.experience[skill.ordinal()] >= MAX_EXPERIENCE) {
			int itemId = MASTER_SKILLCAPES[skill.ordinal()];
			if(itemId != -1) {
				int tab = Bank.getTabForItem2(player, itemId);
				player.getBank(tab == -1 ? 0 : tab).add(itemId, 1);
			}
		}
		if(this.skills.experience[skill.ordinal()] >= MAX_EXPERIENCE) {
			Achievements.finishAchievement(player, AchievementData.REACH_MAX_EXP_IN_A_SKILL);
		...
		if (newLevel > startingLevel) {
			...
			if(newLevel == 99) {
				player.getPointsHandler().incrementLoyaltyPoints(2500);
			} else {
				player.getPointsHandler().incrementLoyaltyPoints(level * 5);
			}
			/*
	...
	public static int[] MASTER_SKILLCAPES = new int[MAX_SKILLS];
	
	static {
		Arrays.fill(MASTER_SKILLCAPES, -1);
		MASTER_SKILLCAPES[Skill.AGILITY.ordinal()] = 13356;
		MASTER_SKILLCAPES[Skill.ATTACK.ordinal()] = 13657;
		MASTER_SKILLCAPES[Skill.CONSTRUCTION.ordinal()] = 13658;
		MASTER_SKILLCAPES[Skill.COOKING.ordinal()] = 13359;
		MASTER_SKILLCAPES[Skill.CRAFTING.ordinal()] = 13360;
		MASTER_SKILLCAPES[Skill.DEFENCE.ordinal()] = 13361;
		MASTER_SKILLCAPES[Skill.FARMING.ordinal()] = 13662;
		MASTER_SKILLCAPES[Skill.FIREMAKING.ordinal()] = 13664;
		MASTER_SKILLCAPES[Skill.FISHING.ordinal()] = 13665;
		MASTER_SKILLCAPES[Skill.FLETCHING.ordinal()] = 13666;
		MASTER_SKILLCAPES[Skill.HERBLORE.ordinal()] = 13667;
		MASTER_SKILLCAPES[Skill.HUNTER.ordinal()] = 13356;
		MASTER_SKILLCAPES[Skill.MAGIC.ordinal()] = 13669;
		MASTER_SKILLCAPES[Skill.MINING.ordinal()] = 13671;
		MASTER_SKILLCAPES[Skill.PRAYER.ordinal()] = 13672;
		MASTER_SKILLCAPES[Skill.RANGED.ordinal()] = 13673;
		MASTER_SKILLCAPES[Skill.RUNECRAFTING.ordinal()] = 13674;
		MASTER_SKILLCAPES[Skill.SLAYER.ordinal()] = 13675;
		MASTER_SKILLCAPES[Skill.SMITHING.ordinal()] = 13676;
		MASTER_SKILLCAPES[Skill.STRENGTH.ordinal()] = 13690;
		MASTER_SKILLCAPES[Skill.THIEVING.ordinal()] = 13679;
		MASTER_SKILLCAPES[Skill.WOODCUTTING.ordinal()] = 13680;
	}
