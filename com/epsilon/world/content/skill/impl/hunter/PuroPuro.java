...
import com.epsilon.engine.task.impl.NPCRespawnTask;
...
public class PuroPuro {
	...
	public static void catchImpling(Player player, final NPC imp) {
		...
		int extra = 0;
		if(player.getEquipment().containsAll(Herblore.ITEMS)) {
			extra += 10;
		}
		boolean sucess = player.getSkillManager().getCurrentLevel(Skill.HUNTER) > 8 ? Misc.getRandom(player.getSkillManager().getCurrentLevel(Skill.HUNTER) + extra / 2) > 1 : true;
		if(sucess) {
			if(imp.isRegistered()) {
				...
				if(player.getGameMode() == GameMode.EXTREME) {
					player.getSkillManager().addExperience(Skill.HUNTER, (int) (implingData.XPReward*0.25));
				} else {
				...
				if(player.getEquipment().containsAll(Herblore.ITEMS)) {
					if(player.getGameMode() == GameMode.EXTREME) {
						player.getSkillManager().addExperience(Skill.HUNTER, (int) ((implingData.XPReward * 1.5)*0.25));
					} else {
						player.getSkillManager().addExperience(Skill.HUNTER, (int) (implingData.XPReward * 1.5));
					}
				...
				if(random >= 1 && random <= 3) {
					Misc.giveReward(player, Herblore.ITEMS);
				}
			}
		} else
