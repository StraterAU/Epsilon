		...
		player.setCurrentTask(new Task(2, player, true) {
			...
			public void execute() {
				...
				if(player.getGameMode() == GameMode.EXTREME) {
					player.getSkillManager().addExperience(Skill.FLETCHING, shafts ? 80 : (int) ((bow.getXp() * 2.5)*0.25));
				} else {
			...
			if (log == g.unStrung()) {
				...
				player.setCurrentTask(new Task(2, player, false) {
					...
					public void execute() {
						...
						if(player.getGameMode() == GameMode.EXTREME) {
							player.getSkillManager().addExperience(Skill.FLETCHING, (int) ((int) (g.getXP() * 2)*0.25));
						} else {
		...
		if (arr != null) {
			if (player.getSkillManager().getCurrentLevel(Skill.FLETCHING) >= arr.getLevelReq()) {
				if (player.getInventory().getAmount(arr.getItem1()) >= 15 && player.getInventory().getAmount(arr.getItem2()) >= 15) {
					...
					if(player.getGameMode() == GameMode.EXTREME) {
						player.getSkillManager().addExperience(Skill.FLETCHING, (int) ((arr.getXp() * 2.5)*0.25));
					} else {
