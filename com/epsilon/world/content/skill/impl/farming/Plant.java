...
public class Plant {
			...
			TaskManager.submit(new Task(1, player, true) {
				...
				public void execute() {
					...
					if(player.getGameMode() == GameMode.EXTREME) {
						player.getSkillManager().addExperience(Skill.FARMING, (int) ((int) Plants.values()[plant].harvestExperience*0.25));
					} else {
	...
	public void process(Player player) {
		...
		if(elapsed > 0) {
			if(player.hasRights(PlayerRights.SUPER_DONATOR) || player.hasRights(PlayerRights.EXTREME_DONATOR)) {
				elapsed += (grow / 100) * 25;
			} else if(player.hasRights(PlayerRights.LEGENDARY_DONATOR) || player.hasRights(PlayerRights.UBER_DONATOR)) {
				elapsed += (grow / 100) * 50;
			}
		}
		
		if (elapsed >= grow) {
			for (int i = 0; grow > 0 && i < elapsed / grow; i++) {
				/*if (isDiseased()) {
				/*} else 
				if (!isWatered()) {
