package com.epsilon.world.content.skill.impl.dungeoneering;
...
import com.epsilon.world.content.dialogue.impl.DungPartyInvitation;
...
public class DungeoneeringParty {
	...
	public void invite(Player p) {
		...
		if(!p.hasRights(PlayerRights.DEVELOPER) && System.currentTimeMillis() - getOwner().getMinigameAttributes().getDungeoneeringAttributes().getLastInvitation() < 2000) {
	...
	public void remove(Player p, boolean resetTab, boolean fromParty) {
		...
		if(hasEnteredDungeon) {
			...
			int exp = (int) (damage > 0 ? (p.getMinigameAttributes().getDungeoneeringAttributes().getDamageDealt() * 5.2) - (deaths * 2500 + Misc.getRandom(400)) : 0);
			int tokens = (int) (damage > 0 ? (p.getMinigameAttributes().getDungeoneeringAttributes().getDamageDealt() / 7.2) - (deaths * 60 + Misc.getRandom(25)) : 0);
			
			if(killedBoss) {
				exp += 9000 + Misc.getRandom(4500);
				tokens += Misc.getRandom(700);
			}
			if(exp > 0 && tokens > 0) {
				...
				if(p.getGameMode() == GameMode.EXTREME) {
					p.getSkillManager().addExperience(Skill.DUNGEONEERING, (int) (exp*0.25));
				} else {
