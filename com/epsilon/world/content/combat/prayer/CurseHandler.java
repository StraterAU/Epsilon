package com.epsilon.world.content.combat.prayer;
...
public class CurseHandler {
	...
	private static void startDrain(final Player player) {
		...
		TaskManager.submit(new Task(1, player, true) {
			@Override
			public void execute() {
				if((player.hasRights(PlayerRights.EXTREME_DONATOR) || player.hasRights(PlayerRights.LEGENDARY_DONATOR) || player.hasRights(PlayerRights.UBER_DONATOR)) && player.getLocation() != Location.WILDERNESS)
					return;
				if (player.getSkillManager().getCurrentLevel(Skill.PRAYER) <= 0) {
					for (CurseData curse : CurseData.values()) {
						if (player.getCurseActive()[curse.ordinal()]) {
