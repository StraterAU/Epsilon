 package com.epsilon.world.content.combat.weapon;
...
public enum CombatSpecial {
	...
	ABYSSAL_WHIP(new int[] { 4151, 5432, 15441, 15442, 15443, 15444, 20050 }, 50, 1, 1, CombatType.MELEE, WeaponInterface.WHIP) {
		@Override
		public CombatContainer container(Player player, Character target) {
	...
	ARMADYL_CROSSBOW(new int[] { 13655 }, 50, 1, 5, CombatType.RANGED, WeaponInterface.CROSSBOW) {
		@Override
		public CombatContainer container(Player player, Character target) {

			new Projectile(player, target, 5, 44, 3, 43, 31, 0).sendProjectile();
	...
	DRAGON_SPEAR(new int[] { 1249, 1263, 5716, 5730, 13988, 13989, 13990 }, 25, 1, 1, CombatType.MELEE, WeaponInterface.SPEAR) {
		@Override
		public CombatContainer container(Player player, Character target) {
			...
			return new CombatContainer(player, target, 1, CombatType.MELEE,
				...
				public void onHit(int damage, boolean accurate) {
					...
					final Item weapon = player.getEquipment().get(Equipment.WEAPON_SLOT);
					TaskManager.submit(new Task(1, false) {
						@Override
						public void execute() {
							target.getMovementQueue().freeze(weapon != null && weapon.getId() == 13988 || weapon.getId() == 13989 || weapon.getId() == 13990 ? 12 : 6);
							stop();
						}
					});
				}
	...
	public static void assign(Player player) {
		...
		//System.out.println("hello hello 123");
