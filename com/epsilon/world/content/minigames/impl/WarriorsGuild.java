...
public class WarriorsGuild {
	...
	public static boolean itemOnAnimator(final Player player, final Item item, final GameObject object) {
		if(player.getMinigameAttributes().getWarriorsGuildAttributes().hasSpawnedArmour() && !player.hasRights(PlayerRights.DEVELOPER)) {
							...
							if(items != null) {
								...
								TaskManager.submit(new Task(2) {
									...
									public void execute() {
										...
										player.getMovementQueue().freeze(1);
										stop();
									}
								});
							}
	...
	public static int getDefender(Player player) {
		...
		if (foundIndex == 0 && Misc.getRandom(6) < 1) //Chance to receive dragon defender as first defender.
			foundIndex = 7;
	 ...
	 * @param RegularPlayer		The player to show the dialogue for acording to their stats.
	 */
