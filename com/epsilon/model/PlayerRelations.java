...
public class PlayerRelations {
	...
	public PlayerRelations updateLists(boolean online, boolean login) {
		...
		for (Player players : World.getPlayers()) {
			...
			if (players.getRelations().friendList.contains(player.getLongUsername())) {
				...
				players.getPacketSender().sendFriend(player.getLongUsername(), temporaryOnlineStatus ? 2 : 0);
			}
			boolean tempOn = true;
			if (player.getRelations().friendList.contains(players.getLongUsername())) {
				...
				player.getPacketSender().sendFriend(players.getLongUsername(), tempOn ? (online ? 3 : 2) : 0);
			}
		}
		return this;
	}
	...
	public void sendFriends() {
		for(int i = 0; i < player.getRelations().getFriendList().size(); i++) {
			if(World.getPlayerByLong(player.getRelations().getFriendList().get(i)) != null) {
				player.getPacketSender().sendFriend(player.getRelations().getFriendList().get(i), 3);
			} else {
	...
	public void addFriend(Long username) {
		...
		if (friendList.contains(username)) {
			...
			player.getPacketSender().sendFriend(username, friend != null ? 2 : 0);
			if (friend != null) {
	...
	public void message(Player friend, byte[] message, int size) {
		...
		friend.getPacketSender().sendPrivateMessage(player.getLongUsername(), player.getHighestRights(), message, size);
	}
