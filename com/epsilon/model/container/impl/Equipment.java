...
public class Equipment extends ItemContainer {
	...
	public ItemContainer refreshItems() {
		...
		Item item = getPlayer().getEquipment().get(Equipment.WEAPON_SLOT);
		if(item != null && item.getId() == 10501) {
			getPlayer().getPacketSender().sendInteractionOption("Pelt", 2, true);
		} else if(getPlayer().getPlayerInteractingOption() == PlayerInteractingOption.ATTACK && !getPlayer().getInteractionOption().equals("Pelt")) {
			getPlayer().getPacketSender().sendInteractionOption("Attack", 2, true);
		} else {
			getPlayer().getPacketSender().sendInteractionOption("null", 2, true);
		}
		return this;
	}
