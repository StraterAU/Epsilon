...
public class Bank extends ItemContainer {
	...
	public int totalCapacity() {
		return capacity() * getPlayer().getBanks().length;
	}
	
	public int usedCapacity() {
		int total = 0;
		for(int i = 0; i < getPlayer().getBanks().length; i++) {
			Bank bank = getPlayer().getBanks()[i];
			if(bank != null) {
				total += bank.getValidItems().size();
			}
		}
		return total;
	}
	...
	public Bank refreshItems() {
		...
		getPlayer().getPacketSender().sendString(22033, "" + usedCapacity());
		getPlayer().getPacketSender().sendString(22034, "" + totalCapacity());
	...
	public static int getTabForItem2(Player player, int itemID) {
		if(ItemDefinition.forId(itemID).isNoted()) {
			itemID = Item.getUnNoted(itemID);
		}
		for(int k = 0; k < 9; k++) {
	...
	public static class BankSearchAttributes {
		...
		public static void stopSearch(Player player, boolean openBank) {
			...
			player.setCurrentBankTab(0).setNoteWithdrawal(false);
			player.getPacketSender().sendString(27002, Integer.toString(0)).sendString(27000, "1").sendConfig(117, 0).sendString(5383, "The Bank of Epsilon");
			if(openBank)
