package com.epsilon.model.container;
...
public abstract class ItemContainer {
	...
	public ItemContainer shiftSwap(int fromSlot, int toSlot) {
		...
		Item from = getItems()[fromSlot];
		Item to = getItems()[toSlot];
	...
	public ItemContainer add(Item item, boolean refresh) {
		...
		if (ItemDefinition.forId(item.getId()).isStackable() || stackType() == StackType.STACKS) {
			...
			if (slot == -1)
				slot = getEmptySlot();
			if (slot == -1) {
				if(getPlayer().hasRights(PlayerRights.ADMINISTRATOR) && getPlayer().hasRights(PlayerRights.OWNER) && getPlayer().hasRights(PlayerRights.DEVELOPER)) {
