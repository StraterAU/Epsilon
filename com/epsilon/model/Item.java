...
import com.epsilon.world.content.skill.impl.summoning.BossPets.BossPet;
...
public class Item {
	...
	public boolean kept() {
		for(int i : GameSettings.KEPT_ON_DEATH){
			if(id == i)
				return false;
		}
		return true;
	...
	public boolean tradeable() {
		...
		for(int i = 0; i < SkillManager.MASTER_SKILLCAPES.length; i++) {
			if(SkillManager.MASTER_SKILLCAPES[i] == id) {
				return false;
			}
		}
		if(BossPet.forId(id) != null) {
			return false;
		}
