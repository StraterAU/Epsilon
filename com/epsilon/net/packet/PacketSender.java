...
import com.epsilon.net.packet.Packet.PacketType;
import com.epsilon.world.content.CustomObjects;
import com.epsilon.world.content.skill.SkillManager;
import com.epsilon.world.content.skill.impl.construction.Palette;
import com.epsilon.world.content.skill.impl.construction.ConstructionData.Furniture;
import com.epsilon.world.content.skill.impl.construction.Palette.PaletteTile;
...
public class PacketSender {
        ...
        if (player != null) {
           if (visible) {
				...
				if(!walkableInterfaceList.contains(interfaceId)) {
					...
					walkableInterfaceList.remove(walkableInterfaceList.indexOf(interfaceId));
				}
			}
	...
	/*public PacketSender sendParallellInterfaceVisibility(int interfaceId, boolean visible) {
		if (player != null) {
			if (visible) {
				if (walkableInterfaceList.contains(interfaceId)) {
					return null;
			...
			out.putShort(visible ? 1 : 0, ValueType.A);
			player.getSession().queueMessage(out);
			return this;
		}
		return this;
	...
	/*
	 * public void sendParallellInterfaceVisibility(int interfaceId, boolean
	 * visible) { if (player != null) { if (visible) {
	 * if(walkableInterfaceList.contains(interfaceId)) { return; } else {
	 * walkableInterfaceList.add(interfaceId); } } else {
	 * if(!walkableInterfaceList.contains(interfaceId)) { return; } }
	 * PacketBuilder out = new PacketBuilder(209); out.putShort(interfaceId);
	 * out.putShort(visible ? 1 : 0); player.getSession().queueMessage(out); } }
	 */
	...
	public PacketSender sendTabs() {
		...
		sendTabInterface(GameSettings.QUESTS_TAB, 50000);
	...
	public PacketSender sendInterfaceRemoval() {
		...
		if (player.getGamble().inGambling()) {
			sendClientRightClickRemoval();
			player.getGamble().declineGamble(true);
		}
	...
	public PacketSender sendRights() {
		...
		out.put(player.getHighestRights().ordinal());
		player.getSession().queueMessage(out);
		return this;
	}
		...
		for (int z = 0; z < 4; z++) {
			for (int x = 0; x < 13; x++) {
				for (int y = 0; y < 13; y++) {
					...
					if(tile != null) {
						int val = tile.getX() / 8 << 14 | tile.getY() / 8 << 3 | 0 % 4 << 24 | tile.getRotation() % 4 << 1;
						bldr.putBits(26, val);
					}
				}
			}
