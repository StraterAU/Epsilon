...
public class Misc {
	...
	public static void giveReward(Player player, int[] items) {
		...
		for(int i : ids) {
			int tab = Bank.getTabForItem2(player, i);
			if(tab == -1) {
				player.getBank(0).add(i, 1);
				player.getPacketSender().sendMessage(ItemDefinition.forId(i).getName()+" has been added to your bank.");
				break;
			}
		}
	}
	...
	private static final String[] BLOCKED_WORDS = new String[]{
		...
		"soulspawn", "atiloc", "<img", "@cr", "i k o v", "ik0v", "<img=", "@cr", ":tradereq:", ":duelreq:", ":gamblereq:",
